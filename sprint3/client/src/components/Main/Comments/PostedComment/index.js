...
class PostedComment extends React.Component {
  ...
  deleteButton = props => {
    if (!this.state.hover) {
      ...
      return (
        <Button
          class={"buttons content__delete content__delete--hover"}
          display={"Delete"}
  ...
  timeStamp = date => {
    ...
    if (currentYear === postedYear) {
      ...
      let dayDiff = Math.floor(
        Math.abs(pageDate - postedDate) / 1000 / 60 / 60 / 24
      );
      let hourDiff = Math.floor(
        Math.abs(pageDate - postedDate) / 1000 / 60 / 60
      );
    ...
    return (
      ...
      >
        <div className="comments__img">
          <img
            className="comments__img--small"
            src="https://www.fillmurray.com/54/54"
            alt="fillMurray"
          />
        </div>
        <div className="content">
          ...
          <div className="content__flex">
            {/**Start of the username and date wrapper */}
            <p className="content__username">{name}</p>
            <p className="content__date">{this.timeStamp(date)}</p>
            {/**This generates a new timestamp for each post everytime a comment is posted */}
          </div>
          {/**End of username and date wrapper */}
          <div>
